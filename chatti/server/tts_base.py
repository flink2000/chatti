# Modified copy of core/providers/tts/base.py
...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        self.tts_text_buff = []
        # chatti patch: do not split the answer at all.
        #
        # Upstream chops the LLM stream into segments and synthesizes each one
        # separately, to get the first audio out sooner. Two problems here:
        #
        #   1. The lists below held only CJK punctuation plus a few ASCII marks -
        #      the plain ASCII full stop "." was missing entirely. A German
        #      sentence ends with it, so after the first segment nothing ever
        #      split again and the rest piled up anyway.
        #   2. The first segment was allowed to break at a comma, which cut a
        #      German subordinate clause off mid sentence ("Das Wasser," out of
        #      "Das Wasser, das Sie in Flaschen kaufen, stammt aus ...").
        #
        # Segmented playback also cannot be gapless here: the LLM generates
        # slower than the result is spoken, so the audio always catches up with
        # the text and waits. Empty tuples mean _get_segment_text() never finds a
        # boundary; _process_remaining_text_stream() then synthesizes the whole
        # answer once at SentenceType.LAST and it plays in one continuous run.
        #
        # Cost: the first sound arrives only after the LLM has finished. That is
        # decision 8 - answer quality over latency - applied deliberately.
        self.punctuations = ()
    ...
    def _chatti_duration_ms(self, source, file_type):
        """chatti patch: exact speaking duration of one synthesized sentence.

        The device needs it to pace the on-screen text against the voice. Without
        it the subtitle scrolls on its own clock and drifts away from what is
        being said. pydub already parses the audio here, so this costs a second
        decode of a few seconds of speech - nothing next to the LLM round trip.
        Returns None on any failure; the device then falls back to its own timing.
        """
        try:
            ...
            if isinstance(source, (bytes, bytearray)):
                audio = AudioSegment.from_file(
                    BytesIO(source), format=file_type, parameters=["-nostdin"]
                )
            else:
                audio = AudioSegment.from_file(source, parameters=["-nostdin"])
            return int(len(audio))
        except Exception as e:
            logger.bind(tag=TAG).warning(f"chatti: could not measure audio duration: {e}")
            return None
        ...
        if self.delete_audio_file:
            ...
            while max_repeat_time > 0:
                try:
                    ...
                    if audio_bytes:
                        # 使用原始文本用于显示/上报
                        # chatti patch: carry the speaking duration alongside the text
                        self.tts_audio_queue.put((SentenceType.FIRST, None, original_text, getattr(self, 'current_sentence_id', None), self._chatti_duration_ms(audio_bytes, self.audio_file_type)))
            ...
            try:
                ...
                # chatti patch: carry the speaking duration alongside the text
                self.tts_audio_queue.put((SentenceType.FIRST, None, original_text, getattr(self, 'current_sentence_id', None), self._chatti_duration_ms(tmp_file, self.audio_file_type)))
        ...
        while not self.conn.stop_event.is_set():
            ...
            try:
                try:
                    ...
                    # chatti patch: 5-tuple carries duration_ms; every other
                    # provider still pushes 3- or 4-tuples and keeps working.
                    duration_ms = None
                    if len(item) == 5:
                        sentence_type, audio_datas, text, sentence_id, duration_ms = item
                    elif len(item) == 4:
                ...
                future = asyncio.run_coroutine_threadsafe(
                    sendAudioMessage(self.conn, sentence_type, audio_datas, text, sentence_id, duration_ms),
                    self.conn.loop,
                )
                future.result()
