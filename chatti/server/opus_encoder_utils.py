# Modified copy of core/utils/opus_encoder_utils.py
...
import logging
import os  # chatti patch: read CHATTI_OPUS_BITRATE
import traceback
import numpy as np
...
class OpusEncoderUtils:
    ...
    def __init__(self, sample_rate: int, channels: int, frame_size_ms: int):
        ...
        # 比特率和复杂度设置
        # chatti patch: upstream hardcodes 24000 bps, which is sized for cellular
        # links. At a 24 kHz sample rate Opus then has to narrow its bandwidth,
        # and sibilants (s, sch, z) break up into a hiss that is easy to mistake
        # for a bad voice or a noisy amplifier. On a LAN the bytes are free.
        # Override via env so the value stays visible without editing code.
        self.bitrate = int(os.environ.get("CHATTI_OPUS_BITRATE", 48000))  # bps
