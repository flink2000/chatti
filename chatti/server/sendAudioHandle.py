# Modified copy of core/handle/sendAudioHandle.py
...
from core.utils.chatti_log import log_turn  # chatti patch: transcript recorder
...
async def sendAudioMessage(conn: "ConnectionHandler", sentenceType, audios, text, sentence_id=None, duration_ms=None):
    # chatti patch: duration_ms is the exact speaking length of this sentence,
    # measured in tts/base.py. It travels with sentence_start so the device can
    # pace its subtitle against the voice instead of against a fixed timer.
    # 跳过旧句子残留音频
    ...
    if sentenceType == SentenceType.FIRST:
        ...
        if (
            ...
            conn.audio_rate_controller.add_message(
                lambda: send_tts_message(conn, "sentence_start", text, duration_ms)
            )
        else:
            # 新句子或流控器未初始化，立即发送
            await send_tts_message(conn, "sentence_start", text, duration_ms)
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None, duration_ms=None):
    """发送 TTS 状态消息"""
    ...
    # chatti patch: exact speaking length, so the device can pace its subtitle
    if state == "sentence_start" and duration_ms:
        message["duration_ms"] = int(duration_ms)

    # chatti patch: record the answer. sentence_start is the only marker that
    # means "this text is about to be spoken"; sentence segmentation is off
    # (empty `punctuations` in tts_base.py), so one answer produces one line.
    if state == "sentence_start" and text:
        log_turn(conn, "assistant", text, duration_ms)
...
async def send_stt_message(conn: "ConnectionHandler", text):
    ...
    log_turn(conn, "user", stt_text)  # chatti patch: record the question
