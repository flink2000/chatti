# Modified copy of core/websocket_server.py
...
# chatti patch: registry of live device connections.
# Upstream keeps no list of them — _handle_connection creates a ConnectionHandler,
# hands it to handle_connection() and drops the reference, so nothing outside can
# tell whether a device is connected. Our control panel needs exactly that, so we
# keep the handlers here while they live. core/http_server.py reads this set and
# serves it as /chatti/status. A set of live objects only; nothing is copied, and
# entries are removed in the finally block below, including on a crash.
CHATTI_CONNECTIONS = set()
...
class WebSocketServer:
    ...
    async def _handle_connection(self, websocket: websockets.ServerConnection):
        ...
        CHATTI_CONNECTIONS.add(handler)  # chatti patch: see registry above
        try:  # chatti patch: remember the device beyond this conversation
            from core.utils.chatti_log import remember_device

            remember_device(
                headers.get("device-id"),
                websocket.request.headers.get("x-real-ip")
                or (websocket.remote_address[0] if websocket.remote_address else None),
            )
        except Exception:
            pass
        try:
            ...
            CHATTI_CONNECTIONS.discard(handler)  # chatti patch
            # 强制关闭连接（如果还没有关闭的话）
            try:
                # 安全地检查WebSocket状态并关闭
