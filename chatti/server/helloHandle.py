# Modified copy of core/handle/helloHandle.py
...
async def handleHelloMessage(conn: "ConnectionHandler", msg_json):
    ...
    if audio_params:
        ...
        # chatti patch: do not let the client's *input* rate dictate our *output*
        # rate. The ESP32 announces 16000 (its microphone Opus rate) but plays
        # back at 24000. Upstream copies the whole block, so the server tells the
        # device "I send 16 kHz" while still encoding at conn.sample_rate (24000
        # from config). The device then decodes at 16 kHz and resamples up —
        # audible as a dull, noisy answer, and it logs
        # "Server sample rate 16000 does not match device output sample rate 24000".
        # Keep the configured output rate, take everything else from the client.
        params = dict(audio_params)
        configured_rate = conn.welcome_msg.get("audio_params", {}).get("sample_rate")
        if configured_rate:
            params["sample_rate"] = configured_rate
        conn.welcome_msg["audio_params"] = params
