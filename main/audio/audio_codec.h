...
protected:
    ...
    // chatti: upstream default is 70, too quiet on this board's small speaker.
    // Set back to 100 on request. Note that the TTS already arrives at -0.1 dBFS
    // peak, so full amplifier gain has no headroom left - if the answer sounds
    // harsh or hissy again, this value is the first suspect, not the codec.
    // Only applies while NVS holds no value; SetOutputVolume persists any later
    // change and wins from then on.
