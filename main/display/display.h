...
public:
    ...
    // chatti: "sentence_start" carries the exact speaking length of the sentence,
    // which lets a display pace its subtitle against the voice instead of a fixed
    // timer. duration_ms is 0 when the server does not report it. The default
    // keeps the previous behaviour for every other display.
    virtual void SetSpokenSentence(const char* text, uint32_t duration_ms) {
