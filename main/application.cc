...
// chatti: let the network tell us where the server is.
//
// `ota_url` lives in NVS and used to be typed in by hand on the setup page.
// That is one of the four setup steps, and the one that silently rots: when the
// router hands the PC a different address, the device keeps dialling the old
// one and nothing notices. Chatti Control advertises the server over mDNS
// (chatti/control/app/announce.py), so we can simply ask.
//
// `replace_existing` is false on the first call - a hand-typed address is an
// explicit choice and beats discovery. Only once the server has proven
// unreachable is the stored value the prime suspect and gets overwritten.
void ChattiDiscoverServer(bool replace_existing) {
    Settings settings("wifi", true);
    std::string current = settings.GetString("ota_url");
    if (!current.empty() && !replace_existing) {
        return;
    }
    std::string found = chatti::DiscoverOtaUrl();
    if (found.empty() || found == current) {
        return;
    }
    settings.SetString("ota_url", found);
    ESP_LOGI(TAG, "chatti: ota_url discovered via mDNS: %s", found.c_str());
}

}  // namespace

void Application::CheckNewVersion() {
    // chatti: keep asking instead of giving up after 10 tries.
    //
    // Despite the name this is not a version check. The reply carries the
    // websocket address and is written to NVS (ota.cc:167-183), so it is how the
    // device learns where to talk. Without it there is nothing else it can do.
    //
    // Upstream doubled the delay up to 2560 s and then returned for good, so a
    // device switched on before the PC stopped asking after ~85 minutes and had
    // to be power cycled. Cap the delay at a minute and keep trying: while this
    // loop runs the device stays in "starting", where a short BOOT click opens
    // the setup mode.
    const int MAX_RETRY_DELAY = 60;
    int retry_count = 0;
    int retry_delay = 10;  // Initial retry delay in seconds

    // Nothing stored yet? Then this is a fresh device and mDNS is the whole
    // setup step. Costs one multicast query.
    ChattiDiscoverServer(false);
    ...
    while (true) {
        ...
        if (err != ESP_OK) {
            ...
            // Two failures in a row and the stored address is the suspect, not
            // the network. Ask again and take the answer this time.
            if (retry_count == 2) {
                ChattiDiscoverServer(true);
            }
            ...
            ESP_LOGW(TAG, "Server not reachable, retry in %d seconds (attempt %d)", retry_delay,
                     retry_count);
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    ...
                    // chatti: optional, sent by our server so the display can
                    // pace the subtitle against the voice. 0 when absent.
                    auto duration = cJSON_GetObjectItem(root, "duration_ms");
                    uint32_t duration_ms =
                        cJSON_IsNumber(duration) ? (uint32_t)duration->valuedouble : 0;
                    ESP_LOGI(TAG, "<< %s (%" PRIu32 " ms)", text->valuestring, duration_ms);
                    Schedule([display, message = std::string(text->valuestring),
                              glyphs = std::move(glyphs), bpp, duration_ms]() {
                        display->AddTextGlyphs(glyphs, bpp);
                        display->SetSpokenSentence(message.c_str(), duration_ms);
                    });
                }
            }
