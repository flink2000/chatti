...
// chatti: how many times the saved network is tried before the setup hotspot
// opens. Upstream gave up after one attempt, which is wrong for a device that
// sits on a desk: a router that reboots, or a weak link that loses the DHCP
// handshake once, then leaves it waiting for someone to re-enter a password it
// already knows. Three attempts cost three minutes of patience at worst.
...
void WifiBoard::StartNetwork() {
    ...
    // chatti: what someone sees in their phone's WLAN list. The device appends
    // "-<last 2 MAC bytes>", so several boards stay distinguishable.
    config.ssid_prefix = "Chatti Server";
    ...
    //
    // chatti: deliberately NOT derived from ssid_prefix any more. That prefix is
    // now "Chatti Server", and a space is not legal in a DHCP hostname (RFC 1123
    // allows letters, digits and hyphens) - routers may reject the request, and
    // the one thing this device must not lose is its address.
    uint8_t mac[6];
    if (esp_read_mac(mac, ESP_MAC_WIFI_STA) == ESP_OK) {
        char hostname[32];
        snprintf(hostname, sizeof(hostname), "Chatti-%02X%02X", mac[4], mac[5]);
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
        case NetworkEvent::Connected:
            ...
            connect_attempts_ = 0;   // chatti: a good connection clears the count
        ...
        case NetworkEvent::Connecting:
            ...
            // chatti: no modem sleep before we even have an address. ESP-IDF
            // defaults the station to WIFI_PS_MIN_MODEM, and the first thing
            // that has to arrive after association is the DHCP OFFER - a
            // broadcast, which an access point only sends right after a DTIM
            // beacon. On a weak link that is exactly the frame a dozing station
            // drops, and the device then sits on "Verbinde zu ..." forever with
            // no `sta ip:` line at all (measured 2026-08-16 at -78 dBm).
            // Safe here rather than in StartNetwork(): the station is started by
            // now, so esp_wifi_set_ps() cannot fail its ESP_ERROR_CHECK.
...
void WifiBoard::OnWifiConnectTimeout(void* arg) {
    ...
    // chatti: try again before falling back to the hotspot. TryWifiConnect()
    // re-arms this very timer, so the counter is what ends the loop.
    if (++board->connect_attempts_ < CONNECT_ATTEMPTS) {
        ESP_LOGW(TAG, "WiFi connection timeout, retrying (%d/%d)",
                 board->connect_attempts_ + 1, CONNECT_ATTEMPTS);
        board->TryWifiConnect();
        return;
    }

    ESP_LOGW(TAG, "WiFi connection timeout after %d attempts, entering config mode",
             CONNECT_ATTEMPTS);
    board->connect_attempts_ = 0;
